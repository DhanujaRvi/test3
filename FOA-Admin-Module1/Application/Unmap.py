"""
Description     : The class UNMAP serves as a "POST" request functionality to unmap the data 
...
class UNMAP(Resource):
    ...
    def post(self, *args, **kwargs):
        try:
            ...
            kwargs["api"] = "/admin/unmap_"+ self.table
            ...
            if version == 'v3.0':
                response = adapter.unmap(data, admin_id, kwargs, logger)
                ...
                if response == -2:
                    ...
                    error_message = 'Cannot UnMap {} '.format(table)
                    logger.error(error_message)
                    kwargsformation(kwargs, '400', error_message, APIStartTime)
                    DBlogger.info("Exiting "+kwargs["api"]+" API", kwargs)
                    return make_response(jsonify({'message': error_message}), 400)
                    ...
                    message = '{} UnMapped Successfully'.format(table)
                    logger.info(message)
